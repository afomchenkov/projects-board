...
const parseUpdatedAt = (date: string): string => {
...
export const Card = memo<CardType>(({ columnCard }) => {
  const [closestEdge, setClosestEdge] = useState<Edge | null>(null);
  const [state, setState] = useState<DraggableState>("idle");
  const cardRef = useRef<HTMLDivElement | null>(null);

  const { id: cardId, name, progress, updatedAt } = columnCard;
  ...
  return (
    ...
    >
      ...
      <span className="app-board-card__date">{parseUpdatedAt(updatedAt)}</span>
